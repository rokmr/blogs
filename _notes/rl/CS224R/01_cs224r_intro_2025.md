---
title: "CS224R Lecture 1 - Introduction to RL"
date: 2025-01-15
...
**State ($s_t$)**: Complete environment description at time $t$ (e.g., robot position, velocity)
...
**Transition Dynamics**: $p(s_{t+1} | s_t, a_t)$ - probability of next state given current state-action
...
2. Stochastic policy $\pi_\theta(a_t | s_t)$
...
- $\pi(a | s)$: probability distribution over actions given state
- Parameterized: $\pi_\theta(a | s)$
...
Relationship: $V^\pi(s) = \mathbb{E}_{a \sim \pi(\cdot|s)} [Q^\pi(s, a)]$
...
Flow: $s_t \xrightarrow{\pi(\cdot|s_t)} a_t \xrightarrow{p(\cdot|s_t, a_t)} s_{t+1} \rightarrow \tau$
